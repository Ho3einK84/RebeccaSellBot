/** English stock bot copy. */

export const EN_TEXTS: Record<string, string> = {
  onboarding_welcome:
    '👋 *Welcome to Rebecca*\n\nBuy, renew, and manage your VPN service securely in this chat.\n\nChoose your preferred language to get started:',
  ui_status_active: 'Active',
  ui_status_inactive: 'Inactive',
  ui_status_pending: 'Pending',
  ui_status_attention: 'Needs attention',
  ui_status_expired: 'Expired',
  ui_status_error: 'Error',
  welcome:
    '👋 Welcome to Rebecca VPN Bot!\nUse the menu below to buy subscriptions, manage your wallet, and track your services.',
  subscription_view_detail: '👁 View Service',
  subscription_detail_title: '📱 *Subscription Details:* `{username}`',
  subscription_advanced_section: '⚙️ *Advanced Settings:*',
  subscription_danger_section: '⚠️ *Danger Zone:*',
  wallet_dashboard_title: 'User Wallet',
  wallet_pending_receipt_detail:
    '⏳ *Pending Receipt:* \n💰 *Amount:* {amount} Toman\n📅 *Submitted:* {date}\n📌 *Status:* Under admin review',
  topup_preset_prompt: '💬 Please select a preset amount or enter a custom amount below:',
  topup_pending_exists_detail:
    '⚠️ You already have a pending topup receipt ({amount} Toman). You cannot submit another receipt until it is processed.',
  home_title: 'Home — Rebecca Sales Bot',
  home_subtitle: 'Your wallet and service status at a glance',
  home_balance: 'Wallet Balance',
  home_active_services: 'Active Subscriptions',
  home_service_overview: 'Service Overview',
  home_near_expiry_warning: 'Service Requiring Attention',
  home_near_expiry_detail: '`{username}` · {days} {days_unit} remaining',
  home_services_unavailable_hint:
    'Service information is temporarily unavailable. Please try again shortly.',
  home_no_active_services_hint:
    'You do not have a subscription yet. Start with “Buy Subscription”.',
  wallet_dashboard_subtitle: 'Manage top-ups, payment receipts, and gift codes here',
  wallet_available_balance: 'Available Balance',
  wallet_pending_section: 'Receipt Under Review',
  wallet_pending_amount: 'Amount',
  wallet_pending_submitted: 'Submitted',
  wallet_pending_status: 'Status',
  wallet_pending_status_detail: 'Awaiting admin review',
  wallet_dashboard_empty_hint:
    'Choose “Top Up” to add funds, or “Promo Code” to redeem a gift or discount.',
  shop_title: 'Subscription Shop',
  shop_subtitle: 'Choose a plan; you will see the final price before confirming.',
  shop_promo_section: 'Active Promo Code',
  shop_hint: 'Review the plan details and final amount before any purchase is confirmed.',
  trial_preview_heading: 'Rebecca Free Trial',
  trial_preview_subtitle: 'Try the service with a complimentary subscription before you buy',
  trial_traffic_label: 'Included Traffic',
  trial_duration_label: 'Validity',
  trial_terms_label: 'Terms',
  trial_terms: 'Each account can claim this complimentary trial subscription once.',
  insufficient_balance_detail:
    '⚠️ *Insufficient wallet balance.*\n\n💰 *Package price:* {price} Toman\n👛 *Your balance:* {balance} Toman\n📉 *Deficit:* {deficit} Toman',
  insufficient_balance_title: 'Add Funds to Continue',
  insufficient_balance_subtitle: 'Your wallet balance does not cover this package yet.',
  insufficient_balance_deficit_label: 'Amount still needed',
  insufficient_balance_wallet_section: 'Wallet Balance',
  insufficient_balance_price_label: 'Package price',
  insufficient_balance_balance_label: 'Available now',
  insufficient_balance_hint: 'Top up your wallet, then choose this package again.',
  trial_preview_title: '🎁 *Rebecca Free Trial*',
  trial_preview_text:
    '🎁 *Free Trial Preview*\n\n📊 *Traffic:* {gb} GB\n⏳ *Duration:* {days} Days\n\nℹ️ *Terms:* This trial subscription is completely free and can be claimed once per account.',
  trial_start_button: '🎁 Claim Free Trial',
  shop_clear_promo_button: '❌ Remove Promo Code',
  shop_promo_active: '🎟️ *Active Promo:* `{code}`',
  direct_topup_button: '👛 Top Up Wallet',
  currency_toman: 'Toman',
  service_unit: 'Service(s)',
  days_unit: 'Day(s)',
  main_menu: '🏠 Main menu\nPlease select an option:',
  balance:
    '👛 Wallet balance\n\n💰 Available balance: {balance} Toman\n\nUse the buttons below to top up or redeem a promo code.',
  shop: '🛍️ Subscription shop\nChoose a plan below to activate your secure service.',
  topup_instructions:
    '💳 Top up wallet\n\nPlease transfer the amount to the card below and send a clear photo of the receipt:\n\n💳 Card number: {card_number}\n👤 Card holder: {card_holder}',
  topup_title: 'Top Up Wallet',
  topup_subtitle: 'Transfer funds to the card below, then submit one clear receipt for review.',
  topup_payment_section: 'Transfer Details',
  topup_card_number_label: 'Card number',
  topup_card_holder_label: 'Account holder',
  topup_selected_amount_label: 'Selected amount',
  topup_choose_amount_hint: 'Choose a preset amount below, or send a custom amount as a number.',
  topup_receipt_title: 'Submit Receipt',
  topup_receipt_subtitle: 'Send a clear photo of the payment receipt.',
  topup_review_title: 'Review Receipt Submission',
  topup_review_subtitle:
    'Confirm the amount and receipt before placing it in the admin review queue.',
  topup_review_consequence:
    'After submission, you cannot upload another receipt until this one is reviewed.',
  topup_success_title: 'Receipt Submitted',
  topup_success_subtitle:
    'Your wallet will be credited after an administrator reviews this receipt.',
  topup_receipt_id_label: 'Receipt ID',
  topup_pending_title: 'A Receipt Is Already Under Review',
  topup_pending_subtitle: 'You can submit another receipt after this one is approved or rejected.',
  topup_success:
    '✅ Your receipt was submitted successfully. Your wallet will be credited after admin review.',
  config_created:
    '🎉 Your subscription was activated successfully!\n\n🔗 Connection link:\n`{sub_url}`\n\nCopy this link into your VPN client to connect.',
  claimed_success: '✅ Subscription linked to your account successfully.',
  claim_failed: '⚠️ This subscription link is invalid or already claimed.',
  claim_already_claimed: '⚠️ This subscription link is already claimed by another account.',
  claim_rate_limited: '⚠️ Please wait a moment before trying again.',
  insufficient_balance: '⚠️ Insufficient wallet balance. Please top up your account first.',
  user_not_found: '⚠️ User account not found.',
  purchase_failed: '⚠️ Transaction failed. No funds were deducted from your wallet.',
  purchase_outcome_pending: '⏳ Transaction status is being verified. Funds remain reserved.',
  purchase_in_progress: '⏳ A transaction is already processing. Please wait.',

  trial_already_used: '⚠️ You have already used your free trial.',
  trial_success: '🎉 Your free trial subscription is now active!',
  trial_disabled: '⚠️ Free trial service is currently disabled.',
  trial_creation_failed: '⚠️ Failed to create free trial. Please try again later.',

  promo_invalid: '⚠️ Invalid promo or gift code.',
  promo_inactive: '⚠️ This promo code is currently inactive.',
  promo_expired: '⚠️ This promo code has expired.',
  promo_max_uses_reached: '⚠️ This promo code has reached its maximum usage limit.',
  promo_user_max_uses_reached: '⚠️ You have reached your personal usage limit for this promo code.',
  promo_minimum_purchase_not_met: '⚠️ This purchase does not meet the promo code minimum amount.',
  promo_already_redeemed: '⚠️ You have already redeemed this promo code.',
  promo_gift_credit_success: '🎁 Gift credit added to your wallet successfully!',
  promo_redeem_failed: '⚠️ Failed to redeem promo code. Please try again.',
  promo_valid: '✅ Promo code is valid.',
  promo_not_purchase_code:
    '⚠️ This code is a wallet gift credit and cannot be applied to direct purchases.',

  access_denied: '⚠️ You are not authorized to perform this action.',
  rate_limited: '⚠️ Rate limit exceeded. Please wait a moment and try again.',
  admin_access_denied: '⚠️ This section is restricted to administrators.',

  admin_menu_title: '🛠️ Admin Panel\nSelect an option below to manage bot settings and services:',
  admin_home_title: 'Administration',
  admin_home_subtitle: 'Run daily operations, manage sales, monitor panels, and configure the bot.',
  admin_home_hint:
    'Choose an operational area below. Pending work and service health are shown on the buttons.',
  admin_daily_title: 'Daily Operations',
  admin_daily_subtitle:
    'Review receipts, find users, adjust balances, and communicate with customers.',
  admin_sales_title: 'Sales & Packages',
  admin_sales_subtitle: 'Manage packages, custom volume, promo codes, and referral rewards.',
  admin_panels_group_title: 'Rebecca Panels',
  admin_panels_group_subtitle:
    'Monitor panel health, manage targets, and resolve unassigned services.',
  admin_system_title: 'System & Settings',
  admin_system_subtitle: 'Configure bot behavior, administrators, and editable customer copy.',
  admin_group_daily: '⚡ Daily Operations',
  admin_group_sales: '🛍️ Sales & Packages',
  admin_group_panels: '🖥 Rebecca Panels',
  admin_group_system: '⚙️ System & Settings',
  admin_menu_back_to_admin: '◀️ Back to Management',
  admin_sales_packages_button: '📦 Packages Manager',
  admin_sales_custom_volume_button: '✏️ Custom Volume Settings',
  admin_sales_referral_button: '👥 Referrals & Cashback',
  remaining: 'Remaining Traffic',
  expiry: 'Expiration',
  config_not_owned: '⚠️ This subscription does not belong to your account.',
  renewal_in_progress: '⏳ Renewal in progress. Please wait.',
  renewal_package_missing: '⚠️ Selected renewal package is not available.',
  promo_no_longer_usable: '⚠️ Selected promo code is no longer usable.',
  renewing: '⏳ Renewing subscription…',
  renewal_success: '🎉 Subscription `{username}` renewed with package {package_name}.',
  operation_in_progress: '⏳ Operation in progress. Please wait.',
  subscription_link_revoked: '🔐 Previous link revoked. Your new link:\n`{sub_url}`',
  subscription_link_unavailable: '⚠️ Subscription link currently unavailable.',
  subscription_enabled: '✅ Subscription enabled successfully.',
  subscription_disabled: '⏸️ Subscription disabled.',
  config_action_failed: '⚠️ Subscription operation failed.',
  receipt_review_in_progress: '⏳ Your payment receipt is under review.',
  receipt_approved: '✅ Receipt approved and wallet credited.',
  receipt_rejected: '⚠️ Receipt rejected. Contact support if needed.',
  receipt_result_approved_title: 'Wallet Credited',
  receipt_result_rejected_title: 'Receipt Not Approved',
  receipt_result_approved_subtitle:
    'Your payment receipt was approved and the funds are now available in your wallet.',
  receipt_result_rejected_subtitle:
    'Your payment receipt was not approved. Contact support if you need help.',
  receipt_result_amount_label: 'Added to wallet',
  receipt_result_next_hint: 'You can return to the wallet whenever you are ready.',
  claim_handler_failed: '⚠️ Failed to link subscription. Please try again.',
  renewal_button: '🔄 Renew subscription',
  renew_confirm_button: '✅ Confirm and renew',
  renew_select_package: '💬 Select a renewal package for `{username}`:',
  renewal_selection_title: 'Renew Subscription',
  renewal_selection_subtitle:
    'Choose a package for your next renewal. You will review the final total before your wallet is charged.',
  renewal_selection_service_label: 'Service',
  renewal_selection_hint: 'No charge is made until you confirm on the next screen.',
  renewal_review_title: 'Review Renewal',
  renewal_review_subtitle: 'Confirm the service, package, and final wallet charge.',
  renewal_success_title: 'Renewal Complete',
  renewal_success_subtitle: 'The selected package is now active on your service.',
  renewal_success_service_label: 'Service',
  renewal_success_package_label: 'Applied package',
  renewal_processing_title: 'Renewing Your Service',
  renewal_processing_subtitle: 'Please keep this chat open while we apply the selected package.',
  renewal_failed_title: 'We Could Not Complete the Renewal',
  checkout_total_label: 'Amount payable',
  checkout_package_section: 'Selected Package',
  checkout_service_label: 'Service',
  checkout_traffic_label: 'Traffic',
  checkout_duration_label: 'Validity',
  checkout_unit_price_label: 'Price per GB',
  checkout_promo_section: 'Promo Code',
  checkout_confirmation_hint: 'Your wallet will be charged only after you tap “Confirm and renew.”',
  auto_renew_enable_button: '♻️ Enable auto-renew',
  auto_renew_disable_button: '⏹ Disable auto-renew',
  auto_renew_select_package: '💬 Select an auto-renew package:',
  auto_renew_selection_title: 'Set Up Automatic Renewal',
  auto_renew_selection_subtitle:
    'Choose the package to apply automatically when this service is due for renewal.',
  auto_renew_selection_hint:
    'Your wallet is charged only at renewal time, and only if sufficient funds are available.',
  auto_renew_review_title: 'Review Automatic Renewal',
  auto_renew_review_subtitle: 'Confirm the selected package and future wallet charge.',
  auto_renew_charge_label: 'Charge at each renewal',
  auto_renew_review_hint:
    'This does not charge your wallet now. It authorizes future automatic renewals for this service.',
  auto_renew_disable_title: 'Turn Off Automatic Renewal?',
  auto_renew_disable_subtitle: 'Scheduled renewals for this service will stop.',
  auto_renew_disable_consequence: 'You will need to renew this service manually before it expires.',
  auto_renew_enabled_title: 'Automatic Renewal Enabled',
  auto_renew_enabled_subtitle: 'We will use the selected package when this service is due.',
  auto_renew_disabled_title: 'Automatic Renewal Disabled',
  auto_renew_disabled_subtitle: 'This service will no longer renew automatically.',
  auto_renew_confirm:
    'Enable automatic renewal for {username} with {package} at {price} Toman per renewal? Your wallet will be charged when the renewal is due.',
  auto_renew_disable_confirm:
    'Disable automatic renewal for {username}? Scheduled wallet renewals will stop.',
  auto_renew_custom_package: '{gb} GB / {days} days',
  auto_renew_enabled: '✅ Auto-renew is now enabled for this subscription.',
  auto_renew_disabled: '⏹ Auto-renew is now disabled for this subscription.',
  auto_renew_state_enabled: '♻️ *Auto-renew:* enabled — {package}',
  auto_renew_state_disabled: '♻️ *Auto-renew:* disabled',
  auto_renew_package_unavailable: 'Selected package is no longer available',
  auto_renew_package_unavailable_title: 'Automatic Renewal Needs Attention',
  auto_renew_package_unavailable_subtitle:
    'The saved package for this service is no longer available.',
  auto_renew_low_balance:
    '⚠️ Your wallet cannot auto-renew `{username}`, or its selected package is unavailable. Please top up or choose a package again.',
  auto_renew_low_balance_title: 'Automatic Renewal Paused',
  auto_renew_low_balance_subtitle: 'Your wallet balance was not enough for a scheduled renewal.',
  auto_renew_required_label: 'Required for renewal',
  auto_renew_balance_label: 'Wallet balance',
  auto_renew_success:
    '✅ `{username}` was renewed automatically. New quota: {gb} GB; valid until {expiry}.',
  auto_renew_success_title: 'Service Renewed Automatically',
  auto_renew_success_subtitle: 'Your wallet was charged and the service is ready to use.',
  auto_renew_service_label: 'Service',
  auto_renew_package_label: 'Renewal package',
  auto_renew_quota_label: 'New quota',
  auto_renew_expiry_label: 'Valid until',
  renew_custom_button: '✏️ Custom volume ({price} Toman/GB)',
  custom_volume_unavailable:
    '🚫 Custom volume selection is currently disabled. Please choose a predefined package.',
  custom_volume_title: 'Choose Custom Volume',
  custom_volume_subtitle:
    'Set the traffic amount for your subscription; the configured validity period will be applied.',
  custom_volume_input_hint: 'Send a whole-number traffic amount in GB.',
  custom_package_name: '{gb} GB · {days} days',
  purchase_target_unavailable:
    '⚠️ This package target is unavailable. Choose another package or contact support.',
  custom_days_prompt: '💬 Please enter the desired duration in days (e.g. 30):',
  custom_days_invalid:
    '⚠️ Invalid duration entered. Please enter a whole number between 1 and 3650 days.',
  renewal_quote:
    '🔄 Renewal summary for `{username}`\n\n📊 *Traffic:* {gb} GB\n⏳ *Duration:* {days} Days\n💰 *Amount payable:* {amount} Toman ({price_per_gb} Toman/GB)\n\nAre you sure you want to proceed?',
  renewal_quote_with_promo:
    '🔄 Renewal summary for `{username}`\n🎟️ *Promo code:* `{promo_code}`\n\n📊 *Traffic:* {gb} GB\n⏳ *Duration:* {days} Days\n💰 *Amount payable:* {amount} Toman ({price_per_gb} Toman/GB)\n\nAre you sure you want to proceed?',

  menu_buy_subscription: '🛒 Buy subscription',
  menu_wallet: '👛 Wallet',
  menu_my_subscriptions: '📱 My subscriptions',
  menu_free_trial: '🎁 Free trial',
  menu_referral: '👥 Referrals',
  menu_support: '💬 Support',
  menu_custom_amount: '✏️ Custom volume ({price} Toman/GB)',
  buy_confirm_button: '✅ Confirm and purchase',
  menu_back: '◀️ Back',
  menu_close: '✖️ Close',
  navigation_continue_hint: 'Use the button below to continue.',
  menu_cancel: '❌ Cancel',
  menu_top_up: '➕ Top up wallet',
  menu_use_promo: '🎟️ Redeem promo code',
  menu_back_main: '🏠 Main menu',
  menu_language: '🌐 Change language / تغییر زبان',
  language_selection_prompt: '🌐 Please select your preferred language:',
  language_selection_title: 'Choose Your Language',
  language_selection_subtitle: 'You can change this preference again at any time.',
  language_changed: '✅ Bot language changed to English.',
  language_update_failed: '⚠️ Failed to save language preference. Please try again.',
  button_action_failed: '⚠️ Failed to process command. Please try again.',
  button_expired: 'This button is outdated or expired.',
  button_expired_help: '🔄 This page is outdated. Please start the action again from the new menu.',
  operation_cancelled: '✅ Operation cancelled.',
  text_input_required: '⌨️ Please send your text response or tap Cancel.',
  button_input_required: 'Please use one of the buttons shown for this step.',
  photo_input_required: '🖼️ Please send a clear receipt photo or tap Cancel.',
  no_subscriptions: '📭 No subscriptions found in your account.',
  subscriptions_list_complete:
    '✅ {count} subscriptions shown. Use the button below when you are ready to return.',
  subscriptions_loading: 'Loading your subscriptions…',
  subscription_refreshing: 'Refreshing service details…',
  subscription_list_title: 'My Subscriptions',
  subscription_list_subtitle: 'Choose a service to view its connection details and controls.',
  subscription_list_total_label: 'Services in your account',
  subscription_list_page: 'Page {page} of {total_pages}',
  subscription_list_empty_title: 'No subscriptions yet',
  subscription_list_empty_body: 'Your account does not have a subscription yet.',
  subscription_list_empty_action: 'Open the Subscription Shop to get started.',
  subscription_detail_heading: 'Service · `{username}`',
  subscription_detail_live_subtitle: 'Live details from the panel',
  subscription_detail_cached_subtitle: 'Last saved details · panel unavailable',
  subscription_cached_notice:
    'The panel is temporarily unavailable; these are the latest saved values.',
  subscription_status_label: 'Status',
  subscription_usage_section: 'Usage & Validity',
  subscription_connection_section: 'Connection',
  subscription_last_connection_label: 'Last connection',
  subscription_created_label: 'Created',
  subscription_link_label: 'Subscription link',
  subscription_automation_section: 'Automatic Renewal',
  subscription_auto_renew_label: 'Auto-renew',
  subscription_state_active: 'Active',
  subscription_state_disabled: 'Disabled',
  subscription_state_on_hold: 'On hold',
  subscription_state_expired: 'Expired',
  subscription_state_depleted: 'Traffic depleted',
  subscription_state_unknown: 'Unknown ({status})',
  unlimited: '♾️ Unlimited',
  subscription_status:
    '📱 *Subscription:* `{username}`\n\n⚡ *Status:* {status}\n📊 *Remaining Traffic:* {remaining}\n⏳ *Expiration:* {expiry_info}\n🌐 *Last Connection:* {online_info}\n📅 *Created Date:* {created_info}\n\n🔗 *Subscription URL:*\n`{sub_url}`',
  subscription_status_cached:
    '📱 *Cached subscription data:* `{username}`\n\n⚠️ The panel is temporarily unavailable; the latest verified data is shown.\n⚡ *Status:* {status}\n📊 *Remaining Traffic:* {remaining}\n⏳ *Expiration:* {expiry_info}\n🌐 *Last Connection:* {online_info}\n📅 *Created Date:* {created_info}\n\n🔗 *Subscription URL:*\n`{sub_url}`',
  subscription_status_failed: '⚠️ Failed to retrieve status for {username}.',
  subscription_enable_button: '✅ Enable',
  subscription_disable_button: '⏸️ Disable',
  subscription_toggle_button: '🔄 Toggle status',
  subscription_revoke_button: '🔑 Revoke old link',
  subscription_refresh_button: '🔄 Refresh',
  subscription_qr_button: '📷 Get QR',
  subscription_status_active: '✅ Active',
  subscription_status_disabled: '⏸️ Disabled',
  subscription_status_on_hold: '⏳ On hold',
  subscription_status_expired: '⚠️ Expired',
  subscription_status_depleted: '📭 Traffic depleted',
  subscription_status_unknown: 'Unknown: {status}',
  subscription_expiry_never: '♾️ Never expires',
  subscription_expiry_remaining: '{date} ({days} days remaining)',
  subscription_expiry_expired: '{date} (expired)',
  subscription_online_never: '❌ No connection',
  traffic_unit_gb: 'GB',
  subscription_revoke_confirm: 'Revoke the current link for {username} and issue a new one?',
  subscription_revoke_title: 'Replace Subscription Link',
  subscription_revoke_subtitle:
    'Invalidate the current connection link and issue a new private link.',
  subscription_revoke_consequence:
    'Devices using the previous link will need the new link to reconnect.',
  subscription_revoke_success_title: 'New Subscription Link Ready',
  subscription_revoke_success_subtitle: 'The previous link no longer works.',
  subscription_qr_generating: 'Generating QR…',
  subscription_qr_caption: 'Connection QR for {username}',
  subscription_qr_title: 'Connection QR Code',
  subscription_qr_subtitle:
    'Scan this code with your VPN app, or use the subscription link from the service screen.',
  subscription_qr_service_label: 'Service',
  subscription_qr_failed: '⚠️ Could not generate the subscription QR. Please try again.',
  trial_subscription_url:
    '🔗 Your trial subscription link:\n`{sub_url}`\n\nCopy this link into your VPN client.',
  referral_info:
    '👥 Referral Program\n\nEarn rewards when your friends make their first purchase using your link:\n\n🔗 Your referral link:\n`{ref_link}`\n\n💰 Reward per referral: {bonus} Toman',
  referral_title: 'Invite Friends',
  referral_subtitle: 'Earn a reward when a friend makes their first purchase using your link.',
  referral_link_label: 'Your referral link',
  referral_reward_label: 'Reward per first purchase',
  referral_hint:
    'Share this private link with friends. Rewards are added after their first purchase.',
  package_button: '{name} — {price} Toman',
  package_pkg_10gb_30d_name: '10 GB (30 Days)',
  package_pkg_30gb_30d_name: '30 GB (30 Days)',
  package_pkg_50gb_30d_name: '50 GB (30 Days)',
  package_pkg_100gb_60d_name: '100 GB (60 Days)',
  purchase_issuing: '⏳ Issuing {package_name} subscription…',
  purchase_issuing_with_promo:
    '⏳ Issuing {package_name} with promo {promo_code} ({amount} Toman)…',
  purchase_quote:
    '🛒 New subscription purchase summary\n\n📊 *Traffic:* {gb} GB\n⏳ *Duration:* {days} Days\n💰 *Amount payable:* {amount} Toman ({price_per_gb} Toman/GB)\n\nAre you sure you want to proceed?',
  purchase_quote_with_promo:
    '🛒 New subscription purchase summary\n🎟️ *Promo code:* `{promo_code}`\n\n📊 *Traffic:* {gb} GB\n⏳ *Duration:* {days} Days\n💰 *Amount payable:* {amount} Toman ({price_per_gb} Toman/GB)\n\nAre you sure you want to proceed?',
  purchase_review_title: 'Review Purchase',
  purchase_review_subtitle: 'Confirm the selected package and final wallet charge.',
  purchase_confirmation_hint:
    'Your wallet will be charged only after you tap “Confirm and purchase.”',
  purchase_issuing_title: 'Creating Your Service',
  purchase_issuing_subtitle: 'Please keep this chat open while we activate your subscription.',
  purchase_issuing_package_label: 'Selected package',
  purchase_success_title: 'Your Service Is Ready',
  purchase_success_subtitle: 'Copy the private link below into your VPN app to connect.',
  purchase_success_link_label: 'Subscription link',
  purchase_failed_title: 'We Could Not Complete the Purchase',
  custom_gb_prompt: '💬 Please enter the traffic amount in GB:',
  custom_gb_invalid_range: '⚠️ Traffic must be an integer between {min} and {max} GB.',
  custom_gb_simple_prompt: '💬 Please enter the traffic amount in GB:',
  custom_gb_invalid: '⚠️ Invalid traffic amount.',
  promo_prompt: '💬 Please enter the promo or gift code:',
  promo_title: 'Redeem a Code',
  promo_subtitle: 'Enter a promo or gift code to apply it to your wallet or next purchase.',
  promo_input_hint: 'Send the code exactly as you received it.',
  promo_applied_title: 'Code Applied',
  promo_applied_subtitle: 'This code will be used automatically on your next eligible purchase.',
  promo_selected_for_purchase: '✅ Code {promo_code} applied to your next purchase.',
  topup_amount_prompt: '💬 Please enter the top-up amount in Toman:',
  invalid_amount: '⚠️ Invalid amount entered.',
  topup_amount_invalid_range: '⚠️ Amount must be between {min} and {max} Toman.',
  topup_photo_prompt: '🖼️ Please send a clear photo of the payment receipt:',
  topup_confirmation:
    '🧾 *Confirm receipt submission*\n\n💰 Amount: *{amount} Toman*\n🖼️ Receipt image received.\n\nYou cannot submit another receipt until an admin reviews this one.',
  topup_confirm_button: '✅ Submit receipt',
  topup_pending_exists:
    '⏳ You already have a receipt awaiting review. Submit another after it is approved or rejected.',
  topup_failed: '⚠️ Receipt submission failed. Please try again.',

  admin_menu_stats: '📊 Statistics',
  admin_menu_search_user: '🔎 Search user',
  admin_menu_users: '👥 User Management',
  admin_users_title: '👥 *User Management* ({total} total users)\nPage {page} of {total_pages}:',
  admin_users_list_title: 'User Management',
  admin_users_list_subtitle:
    'Find a customer, inspect their account, or take an audited operator action.',
  admin_users_total_label: 'Users in the system',
  admin_users_page_label: 'Page {page} of {total_pages}',
  admin_user_search_button: '🔍 Search User',
  admin_user_btn_format: '👤 {name} | 💰 {balance} Toman',
  admin_menu_settings: '⚙️ Settings Center',
  admin_settings_title: 'Settings Center',
  admin_settings_subtitle: 'Configure sales, payments, and services here.',
  admin_package_manager_title: 'Package Manager',
  admin_package_manager_subtitle: 'Add, edit, or remove packages offered for sale.',
  admin_setting_saved_title: 'Saved',
  admin_package_saved_title: 'Packages Saved',
  admin_menu_texts: '📝 Bot texts',
  admin_menu_manual_topup: '➕ Manual top-up',
  admin_menu_promo_codes: '🎟️ Promo codes',
  admin_menu_create_promo: '➕ Create promo code',
  admin_menu_manage_promo: '⏯️ Manage promo code',
  admin_menu_pending_receipts: '🧾 Pending receipts',
  admin_menu_broadcast: '📣 Broadcast',
  admin_menu_direct_message: '✉️ Direct message',
  admin_menu_management: '🛠️ Management',
  admin_menu_back: '◀️ Back to main menu',
  admin_dashboard_stats:
    '📊 System Statistics\n\n👥 Total Users: {total_users}\n💳 Revenue today/week/month: {daily_revenue} / {weekly_revenue} / {monthly_revenue} Toman\n💰 Total Sales: {total_sales} Toman\n🟢 Active subscriptions: {active_subscriptions}\n⚪ Inactive/expired subscriptions: {inactive_subscriptions}\n🧾 Pending receipts: {pending_receipts}\n🩺 Panel: {panel_health} ({panel_latency} ms)\n🗄️ Database: {database_health} ({database_latency} ms)\n🎁 Referral Rewards: {total_referral_bonus} Toman\n💸 Cashback Paid: {total_cashback} Toman',
  admin_stats_title: 'System Snapshot',
  admin_stats_subtitle: 'Current commercial, operational, and infrastructure indicators.',
  admin_stats_revenue_section: 'Revenue',
  admin_stats_today_label: 'Today',
  admin_stats_week_label: 'This week',
  admin_stats_month_label: 'This month',
  admin_stats_customers_section: 'Customers & Services',
  admin_stats_total_users_label: 'Total users',
  admin_stats_total_sales_label: 'Total sales',
  admin_stats_active_label: 'Active subscriptions',
  admin_stats_inactive_label: 'Inactive or expired',
  admin_stats_pending_receipts_label: 'Receipts awaiting review',
  admin_stats_rewards_section: 'Rewards',
  admin_stats_referral_label: 'Referral rewards paid',
  admin_stats_cashback_label: 'Cashback paid',
  admin_stats_health_section: 'Service Health',
  admin_stats_panel_label: 'Rebecca panels',
  admin_stats_database_label: 'Database',
  admin_stats_latency_unit: 'ms',
  admin_no_promo_codes: '📭 No promo codes found.',
  admin_promo_code_line: '`{code}` — {type}: {value} (uses: {current_uses}/{max_uses}; {active})',
  admin_promo_codes: '🎟️ Promo codes:\n{codes}',
  admin_promo_center:
    '🎟️ Promo code center\n\nTotal: {count} · page {page} of {total_pages}\nSelect a code to manage:',
  admin_promo_center_title: 'Promo Codes',
  admin_promo_center_subtitle:
    'Create, inspect, activate, and retire customer discounts and gifts.',
  admin_promo_total_label: 'Codes in the system',
  admin_promo_page_label: 'Page {page} of {total_pages}',
  admin_promo_detail_title: 'Promo Code Details',
  admin_promo_configuration_section: 'Configuration',
  admin_promo_usage_section: 'Usage & Limits',
  admin_promo_type_label: 'Type',
  admin_promo_value_label: 'Value',
  admin_promo_status_label: 'Status',
  admin_promo_uses_label: 'Uses',
  admin_promo_per_user_label: 'Per user limit',
  admin_promo_min_purchase_label: 'Minimum purchase',
  admin_promo_expiry_label: 'Expires',
  admin_promo_delete_title: 'Delete Promo Code',
  admin_promo_delete_subtitle: 'This code will no longer be available to customers.',
  admin_promo_delete_consequence: 'Deletion is permanent and cannot be undone.',
  admin_promo_create_button: '➕ Create code',
  admin_promo_back_to_list: '◀️ Code list',
  admin_promo_search_button: '🔍 Search codes',
  admin_promo_search_prompt: '🔍 Enter a promo code or part of one:',
  admin_promo_search_results: '🔍 Found {count} matching promo codes:',
  admin_promo_next_page: 'Next ▶️',
  admin_promo_previous_page: '◀️ Previous',
  admin_promo_edit_button: '✏️ Edit',
  admin_promo_open_button: '🎟️ Open code',
  admin_promo_stale_button: '⚠️ This button is stale; the current list is now displayed.',
  admin_promo_detail:
    '🎟️ Code: `{code}`\n\nType: {type}\nValue: {value}\nStatus: {active}\nUses: {current_uses}/{max_uses}\nPer-user limit: {max_uses_per_user}\nMinimum purchase: {min_purchase_amount}\nExpires: {expires_at}',
  admin_promo_never_expires: 'Never',
  admin_promo_activate_button: '✅ Activate',
  admin_promo_deactivate_button: '⏸ Deactivate',
  admin_promo_delete_button: '🗑 Delete code',
  admin_promo_delete_confirm_button: '🗑 Yes, delete',
  admin_promo_delete_confirm: 'Delete `{code}`? Only codes with no redemptions can be deleted.',
  admin_promo_delete_failed:
    '⚠️ `{code}` cannot be deleted because it may already have redemptions.',
  admin_no_pending_receipts: '📭 No pending receipts found.',
  admin_receipt_approve: '✅ Approve',
  admin_receipt_reject: '❌ Reject',
  admin_receipt_approved_confirm:
    '✅ Receipt {receipt_id} approved and user wallet credited successfully.',
  admin_receipt_rejected_confirm: '⚠️ Receipt {receipt_id} rejected.',
  admin_pending_receipt:
    '🧾 Receipt {receipt_id}\nUser: {telegram_id}\nAmount: {amount} Toman\nCreated: {created_at}',
  receipt_already_reviewed: '⚠️ This receipt has already been reviewed.',
  admin_receipt_approve_confirm:
    'Approve receipt {receipt_id}\nUser: {telegram_id}\nAmount: {amount} Toman\n\nCredit the user wallet?',
  admin_receipt_reject_confirm:
    'Reject receipt {receipt_id}\nUser: {telegram_id}\nAmount: {amount} Toman\n\nReject this receipt?',
  admin_receipt_queue_complete: '🧾 Displayed {count} of {total} pending receipts.',
  admin_receipt_batch_button: '✅ Approve this page',
  admin_receipt_batch_confirm: 'Approve {count} receipts with a total amount of {amount} Toman?',
  admin_receipt_batch_confirm_button: '✅ Yes, approve all',
  admin_receipt_batch_done: '✅ Approved {count} receipts in the batch.',
  admin_receipt_queue_title: 'Pending Receipts',
  admin_receipt_queue_subtitle:
    'Review one receipt at a time. Every decision is recorded in the operator audit trail.',
  admin_receipt_queue_pending_label: 'Awaiting review',
  admin_receipt_queue_section: 'Receipts on this page',
  admin_receipt_queue_item_label: 'Receipt {receipt_id} · {amount} Toman',
  admin_receipt_queue_empty_title: 'Receipt Queue Is Clear',
  admin_receipt_queue_empty_body: 'There are no wallet top-up receipts waiting for review.',
  admin_receipt_review_title: 'Review Payment Receipt',
  admin_receipt_review_subtitle: 'Check the payment evidence before changing the customer wallet.',
  admin_receipt_id_label: 'Receipt ID',
  admin_receipt_user_label: 'Customer Telegram ID',
  admin_receipt_amount_label: 'Submitted amount',
  admin_receipt_created_label: 'Submitted at',
  admin_receipt_approve_title: 'Approve Receipt',
  admin_receipt_reject_title: 'Reject Receipt',
  admin_receipt_approve_consequence:
    'Approving immediately credits this amount to the customer wallet and cannot be undone here.',
  admin_receipt_reject_consequence:
    'Rejecting closes this receipt without crediting the customer wallet. The customer can submit a new receipt.',
  admin_receipt_batch_title: 'Approve This Page',
  admin_receipt_batch_subtitle:
    'Only the receipts currently shown below will be approved. Review the evidence first if anything looks unusual.',
  admin_receipt_batch_count_label: 'Receipts to approve',
  admin_receipt_batch_total_label: 'Total wallet credit',
  admin_receipt_batch_result_title: 'Batch Approval Complete',
  admin_receipt_batch_result_subtitle:
    '{count} receipt(s) were approved and the customers were notified.',
  admin_receipt_result_approved_title: 'Receipt Approved',
  admin_receipt_result_rejected_title: 'Receipt Rejected',
  admin_receipt_result_subtitle:
    'The customer has been notified. Return to the queue to continue reviewing.',
  admin_receipt_already_reviewed_title: 'Receipt Already Reviewed',
  admin_target_telegram_id_prompt: '💬 Enter the target Telegram ID:',
  admin_invalid_telegram_id: '⚠️ Invalid Telegram ID.',
  admin_new_balance_prompt: '💬 Enter new user balance:',
  admin_balance_operation_prompt:
    '⚙️ Select wallet operation:\n1) Add funds\n2) Deduct funds\n3) Set balance\nOther) Cancel',
  admin_balance_amount_prompt: '💬 Enter amount to {operation}:',
  admin_balance_add: 'add',
  admin_balance_deduct: 'deduct',
  admin_balance_set: 'set',
  admin_balance_updated: '✅ Balance for user {telegram_id} set to {balance} Toman.',
  admin_balance_update_failed: '⚠️ Failed to update user balance.',
  admin_balance_below_reserved: '⚠️ Balance cannot be lower than reserved purchase funds.',
  admin_balance_confirm:
    'Confirm wallet change\n\nUser: {telegram_id}\nOperation: {operation}\nAmount: {amount} Toman\nCurrent balance: {current_balance} Toman',
  admin_balance_confirm_button: '✅ Confirm wallet change',
  balance_adjusted_notification:
    '💳 Your wallet was updated by an administrator. Current balance: {balance} Toman',
  admin_search_prompt:
    '💬 Enter Telegram ID, @username, UUID, Rebecca service username, or subscription URL:',
  admin_user_not_found: '⚠️ User not found.',
  admin_user_search_found: '✅ User {telegram_id} found.',
  admin_user_open_profile_button: '👤 Open profile',
  admin_user_profile:
    '👤 *User Summary:* `{telegram_id}`\n• *Username:* {username}\n• *Name:* {name}\n• *Status:* {ban_status}\n• *Member since:* {created_at}\n\n👛 *Wallet Details:*\n• *Available Balance:* {balance} Toman\n• *Reserved Balance:* {reserved_balance} Toman\n• *Total Spend:* {total_spend} Toman\n\n📱 *Subscriptions:*\n• *Active Subscriptions:* {active_subscription_count}\n• *Trial Used:* {has_used_trial}\n\n📜 *History & Referrals:*\n• *Referral Code:* `{referral_code}`\n• *Referrer:* {referrer}\n• *Referred Users:* {referred_user_count}\n• *Referral Bonus:* {referral_bonus_earned} Toman\n• *Cashback Earned:* {cashback_earned} Toman\n• *Transactions:* {transaction_count}',
  admin_user_profile_title: 'Customer Profile',
  admin_user_identity_section: 'Identity & Account',
  admin_user_wallet_section: 'Wallet',
  admin_user_services_section: 'Services',
  admin_user_history_section: 'History & Referrals',
  admin_user_id_label: 'Telegram ID',
  admin_user_username_label: 'Username',
  admin_user_name_label: 'Name',
  admin_user_status_label: 'Account status',
  admin_user_result_status_label: 'Status after confirmation',
  admin_user_joined_label: 'Member since',
  admin_user_balance_label: 'Available balance',
  admin_user_reserved_balance_label: 'Reserved balance',
  admin_user_total_spend_label: 'Total spend',
  admin_user_active_services_label: 'Active subscriptions',
  admin_user_trial_label: 'Trial used',
  admin_user_referral_code_label: 'Referral code',
  admin_user_referrer_label: 'Referrer',
  admin_user_referred_count_label: 'Referred users',
  admin_user_referral_bonus_label: 'Referral rewards',
  admin_user_cashback_label: 'Cashback earned',
  admin_user_transactions_label: 'Transactions',
  pagination_previous: '◀️ Previous',
  pagination_next: 'Next ▶️',
  admin_user_ban_button: '🚫 Ban user',
  admin_user_unban_button: '✅ Unban user',
  admin_user_quick_topup_50k: '➕ 50k',
  admin_user_quick_topup_100k: '➕ 100k',
  admin_user_quick_topup_200k: '➕ 200k',
  admin_user_quick_topup_confirm:
    'Credit {amount} Toman to user {telegram_id}? This immediately increases their wallet balance.',
  admin_user_quick_topup_success: '✅ User balance credited by {amount} Toman.',
  admin_user_balance_button: '💳 Custom balance',
  admin_user_subscriptions_button: '📱 Subscriptions',
  admin_user_audit_button: '📜 Audit events',
  admin_user_message_button: '✉️ Send message',
  admin_users_back_button: '◀️ User list',
  admin_user_ban_confirm: 'Ban user {telegram_id}?',
  admin_user_unban_confirm: 'Unban user {telegram_id}?',
  admin_confirm_button: '✅ Confirm',
  admin_user_no_subscriptions: '📭 This user has no subscriptions.',
  admin_user_subscription_card:
    '📱 Subscription `{username}`\nStatus: {status}\nTotal data: {data_limit} GB\nCreated: {created_at}',
  admin_user_subscriptions_complete: '✅ Displayed {count} subscriptions.',
  admin_user_audit_title: '📜 Recent administrative events',
  admin_user_no_audit: 'No administrative events have been recorded.',
  subscription_status_unknown_short: 'Unknown',
  button_refreshed: '🔄 Screen refreshed.',
  admin_username_unset: 'Not set',
  admin_name_unset: 'Not set',
  admin_referrer_none: 'None',
  admin_yes: 'Yes',
  admin_no: 'No',
  admin_banned: 'Banned',
  admin_active: 'Active',
  admin_profile_action_prompt:
    '⚙️ Select action:\n1) Add funds\n2) Deduct funds\n3) Set balance\n4) Ban/Unban\nOther) Cancel',
  admin_invalid_balance: '⚠️ Invalid balance value.',
  admin_new_balance: '💰 New balance: {balance}',
  admin_user_banned: '✅ User {telegram_id} banned.',
  admin_user_unbanned: '✅ User {telegram_id} unbanned.',
  admin_cancelled: '✅ Operation cancelled.',
  admin_settings_list: '⚙️ Dynamic settings:\n{settings}\n\nEnter option number to edit.',
  admin_bootstrap_env: 'Bootstrap env value',
  admin_setting_configured: 'Configured',
  admin_setting_price_per_gb: 'Price per GB',
  admin_setting_card_number: 'Card number',
  admin_setting_card_holder: 'Card holder',
  admin_setting_packages_json: '📦 Predefined packages',
  admin_setting_custom_volume_enabled: '🎚️ Custom volume selection',
  admin_setting_low_traffic_threshold_gb: 'Low-traffic threshold (GB)',
  admin_setting_expiry_warning_days: 'Expiry warning days',
  admin_setting_trial_enabled: 'Trial status',
  admin_setting_trial_gb: 'Trial volume (GB)',
  admin_setting_trial_days: 'Trial validity (days)',
  admin_setting_referral_bonus_toman: 'Referral reward',
  admin_setting_cashback_percent: 'Cashback percent',
  admin_setting_naming_mode: 'Naming method',
  admin_setting_naming_prefix: 'Name prefix',
  admin_setting_custom_naming_template: 'Name template',
  admin_setting_packages_summary: '{count} predefined packages',
  admin_setting_custom_default_days: '⏳ Custom volume validity (days)',
  admin_setting_custom_volume_enabled_prompt:
    'Current status: *{current}*\n\nWhen off, users only see predefined packages.',
  admin_setting_group_pricing: '🛍️ Sales & Packages',
  admin_setting_group_pricing_desc: 'Packages, low-traffic alerts, and the refund window',
  admin_setting_group_custom_volume: '🎚️ Custom Volume',
  admin_setting_group_custom_volume_desc: 'Availability, price per GB, and default validity',
  admin_setting_group_payment: '💳 Payment info',
  admin_setting_group_payment_desc: 'Card number and cardholder name',
  admin_setting_group_trial: '🎁 Free trial',
  admin_setting_group_trial_desc: 'Trial availability, volume, and validity',
  admin_setting_group_referral: '👥 Referral & cashback',
  admin_setting_group_referral_desc: 'Referral reward and purchase cashback',
  admin_setting_group_naming: '🏷️ Subscription names',
  admin_setting_group_naming_desc: 'How new Rebecca subscriptions are named',
  admin_settings_groups_prompt: 'Choose a section.',
  admin_settings_group_prompt: '_{description}_\n\n*Current values*\n{settings}',
  admin_settings_back_groups: '◀️ Sections',
  admin_settings_back_category: '◀️ Back to this section',
  admin_settings_return_category: '↩️ Return to section',
  admin_setting_saved_hint: 'Return to this section to continue.',
  admin_pkg_manager_prompt: '{count} available packages',
  admin_pkg_add: '➕ Add package',
  admin_pkg_default_target: 'Default target',
  admin_pkg_name_prompt: 'Send the package name.\nCurrent: {current}',
  admin_pkg_gb_prompt: 'Send the volume in GB.\nCurrent: {current}',
  admin_pkg_days_prompt: 'Send the validity in days.\nCurrent: {current}',
  admin_pkg_price_prompt: 'Send the price in Toman.\nCurrent: {current}',
  admin_pkg_saved: 'Package changes were saved.',
  admin_pkg_last_removed: 'At least one package must remain.',
  admin_pkg_duplicate_name: 'A package with this name already exists.',
  admin_pkg_limit_reached: 'You can configure up to {count} packages.',
  admin_pkg_page_status: 'Page {page} of {total}',
  admin_pkg_continue_editing: '✏️ Continue editing',
  admin_pkg_cancel_edit: '◀️ Cancel package edit',
  admin_pkg_delete_confirm: 'Delete the “{name}” package? This cannot be undone.',
  config_delete_button: '🗑 Delete / Refund',
  config_delete_confirm_button: '⚠️ Delete permanently',
  config_delete_cancel_button: '❌ Cancel',
  config_delete_warning:
    '⚠️ “Delete forever” will permanently remove subscription `{username}` from BOTH the Rebecca panel and your account.\n\nThis action is irreversible and NO refund will be issued for the remaining balance/time on this subscription.\n\nAre you sure?',
  config_deleted: '✅ Subscription `{username}` was permanently deleted.',
  config_delete_not_found:
    '⚠️ Subscription `{username}` was not found in the database; it may already have been deleted.',
  admin_setting_value_prompt: 'Current value: `{current_value}`\n\nSend the new value.',
  admin_setting_selection_invalid: '⚠️ The settings option is invalid.',
  admin_setting_invalid: 'The value is invalid.',
  admin_setting_number_range_invalid: '⚠️ Enter a number from {min} to {max}.',
  admin_setting_card_invalid: '⚠️ Card number must contain 12–24 digits.',
  admin_setting_support_invalid: '⚠️ Enter a valid Telegram username or numeric ID.',
  admin_setting_naming_prefix_invalid: '⚠️ Enter 1–24 letters, digits, hyphens, or underscores.',
  admin_setting_naming_template_invalid: '⚠️ The template or its variables are invalid.',
  admin_setting_save_failed: '⚠️ Could not save. Try again.',
  admin_setting_apply_failed: '⚠️ Could not apply the change; the previous value was restored.',
  admin_setting_rollback_failed:
    '🚨 The change failed and rollback also failed. Review this setting again.',
  admin_rebecca_url_invalid: '⚠️ The panel URL must be a valid HTTPS URL.',
  admin_setting_saved: '✅ Setting "{setting}" was saved as `{value}`.',
  admin_setting_naming_mode_prompt:
    'Current method: *{current}*\n\nChoose how new service names are generated.',
  admin_setting_naming_mode_prefix_number: '🏷️ Prefix + Counter (e.g. rebecca_1)',
  admin_setting_naming_mode_telegramid_number: '👤 Telegram ID + Counter (e.g. 123456789_1)',
  admin_setting_naming_mode_custom: '🎨 Custom Template (e.g. rebecca_123456789_1)',
  admin_setting_custom_naming_template_prompt:
    'Current template: `{current}`\n\n*Variables*\n• `{code_prefix}` — current prefix: `{prefix_value}`\n• `{code_telegram_id}` — Telegram ID\n• `{code_counter}` — counter\n• `{code_random4}` — random 4-character code\n\n*Examples*\n• `{example_primary}`\n• `{example_random}`\n\nSend the new template.',
  admin_setting_naming_prefix_prompt:
    'Current prefix: `{current}`\n\nSend 1–24 letters, digits, hyphens, or underscores.',
  admin_setting_trial_enabled_prompt:
    'Current status: *{current}*\n\nEnable free trials for new users?',
  admin_setting_trial_enabled_on: '🟢 Enabled',
  admin_setting_trial_enabled_off: '🔴 Disabled',
  admin_setting_naming_mode_val_prefix_number: 'Prefix + Counter',
  admin_setting_naming_mode_val_telegramid_number: 'Telegram ID + Counter',
  admin_setting_naming_mode_val_custom: 'Custom Template',
  admin_setting_enabled_on: '🟢 Enabled',
  admin_setting_enabled_off: '🔴 Disabled',
  admin_promo_code_prompt: '💬 Enter the promo code:',
  admin_promo_code_invalid:
    '⚠️ The code must be 3–128 characters using only letters, numbers, hyphens, or underscores.',
  admin_promo_type_prompt:
    '💬 Enter the promo type: `gift_credit`, `discount_percent`, `discount_fixed`, or `gift_gb`.',
  admin_promo_type_percent: '٪ Percentage',
  admin_promo_type_fixed: '💰 Fixed amount',
  admin_promo_type_credit: '🎁 Wallet credit',
  admin_promo_type_gb: '📊 Gift GB',
  admin_invalid_promo_type: '⚠️ The promo code type is invalid.',
  admin_promo_value_prompt: '💬 Enter the promo code value:',
  admin_invalid_promo_value: '⚠️ The promo code value is invalid.',
  admin_promo_max_uses_prompt: '💬 Enter the maximum number of uses:',
  admin_invalid_promo_max_uses: '⚠️ The maximum number of uses is invalid.',
  admin_promo_per_user_prompt: '💬 Enter the maximum uses per user:',
  admin_invalid_promo_per_user:
    '⚠️ The per-user cap must be positive and no greater than the global cap.',
  admin_promo_min_purchase_prompt: '💬 Enter the minimum purchase amount, or 0 for none:',
  admin_invalid_promo_min_purchase: '⚠️ Minimum purchase must be a non-negative integer.',
  admin_promo_expiry_prompt: '📅 Enter expiry as YYYY-MM-DD, or enter 0 for no expiration:',
  admin_invalid_promo_expiry: '⚠️ Expiry must be a valid future date in YYYY-MM-DD format.',
  admin_promo_active_prompt: 'Choose the code state. Current state: {active}',
  admin_promo_save_summary:
    '🎟️ *Confirm promo code*\n\nCode: `{code}`\nType: {type}\nValue: {value}\nGlobal cap: {max_uses}\nPer-user cap: {max_uses_per_user}\nMinimum purchase: {min_purchase_amount}\nExpires: {expires_at}\nState: {active}',
  admin_promo_save_confirm_button: '✅ Save code',
  admin_promo_save_consequence:
    'Saving can change customer pricing or wallet credit according to this code’s active state.',
  admin_promo_created: '✅ Promo code `{code}` created successfully.',
  admin_promo_updated: '✅ Promo code `{code}` updated successfully.',
  admin_promo_create_failed: '⚠️ Failed to create promo code.',
  admin_promo_manage_prompt: '💬 Enter the promo code to manage:',
  admin_promo_toggle_prompt:
    '💬 Enter the new state:\n1) Active\n2) Inactive\n3) Delete unused code',
  admin_promo_active: 'Active',
  admin_promo_inactive: 'Inactive',
  admin_promo_toggled: '✅ Promo code `{code}` is now {active}.',
  admin_promo_deleted: '✅ Promo code `{code}` was deleted.',
  admin_promo_not_found: '⚠️ Promo code not found.',
  admin_text_editor_intro: '📝 Edit bot copy. Choose the language, category, and key step by step.',
  admin_text_editor_title: 'Translation Editor',
  admin_text_editor_subtitle:
    'Safely review and update bilingual bot copy without restarting the service.',
  admin_text_key_label: 'Translation key',
  admin_text_current_value_label: 'Current value',
  admin_text_default_value_label: 'Default value',
  admin_text_preview_truncated:
    'This preview is shortened; the complete stored value is preserved.',
  admin_text_reset_title: 'Reset Translation Override',
  admin_text_reset_subtitle:
    'Review the custom text before returning this key to its built-in default.',
  admin_text_reset_consequence:
    'The custom override will be removed immediately. The built-in default will be used for future messages.',
  admin_text_saved_title: 'Translation Saved',
  admin_text_key_invalid: '⚠️ The text key is invalid.',
  admin_text_value_prompt:
    '💬 *Edit key copy:* `{key}`\n\n📌 *Current value:*\n{current_value}\n\n📌 *Default value:*\n{default_value}',
  admin_text_edit_prompt: '⌨️ Please send the new text as a message or tap Cancel:',
  admin_text_edit_button: '✏️ Edit text',
  admin_text_reset_button: '🔄 Reset to default',
  admin_text_reset_confirm:
    'Reset the custom text for `{key}` to its default? Your custom version will be removed.',
  admin_text_reset_success: '✅ Text `{key}` has been reset to default successfully.',
  admin_text_value_invalid: '⚠️ The text is empty or too long.',
  admin_text_saved: '✅ Text `{key}` was saved without restarting the bot.',
  admin_text_language_prompt: '💬 Choose the language of the text you want to edit:',
  admin_text_category_prompt: '💬 Choose the category of the text you want to edit:',
  admin_text_key_prompt:
    '💬 Choose the key from the “{category}” category ({count} keys):\n📖 {description}',
  admin_text_key_prompt_search: '💬 Keys in “{category}” matching “{search}” ({count} results):',
  admin_text_search_prompt: '💬 Enter a search term to filter the keys:',
  admin_text_search_button: '🔍 Search',
  admin_text_search_clear: '❌ Clear search',
  admin_text_no_keys_found: '📭 No keys matched your search.',
  admin_text_next_page: 'Next ▶️',
  admin_text_prev_page: '◀️ Previous',
  admin_text_back_categories: '◀️ Categories',
  admin_text_category_admin_menu: '🛠️ Admin menu',
  admin_text_category_admin_settings: '⚙️ Admin settings',
  admin_text_category_admin_balance: '💳 Balance and wallet',
  admin_text_category_admin_promo: '🎟️ Promo codes',
  admin_text_category_admin_receipts: '🧾 Pending receipts',
  admin_text_category_admin_users: '👤 User management',
  admin_text_category_admin_broadcast: '📣 Broadcast',
  admin_text_category_admin_direct: '✉️ Direct message',
  admin_text_category_admin_other: '📦 Other admin texts',
  admin_text_category_menus: '📱 Menus',
  admin_text_category_user_purchase: '🛒 Purchase and subscriptions',
  admin_text_category_user_wallet: '👛 Wallet and top-up',
  admin_text_category_user_general: '💬 General user messages',
  admin_text_cat_desc_admin_menu: '📝 Header text and main options in the admin menu',
  admin_text_cat_desc_admin_settings: '📝 Prompts and instructions for bot settings management',
  admin_text_cat_desc_admin_balance: '📝 Messages for manual balance adjustments and top-ups',
  admin_text_cat_desc_admin_promo: '📝 Prompts for creating, toggling, and listing promo codes',
  admin_text_cat_desc_admin_receipts: '📝 Prompts for reviewing, approving, and rejecting receipts',
  admin_text_cat_desc_admin_users:
    '📝 Prompts for searching users, profile view, and ban management',
  admin_text_cat_desc_admin_broadcast: '📝 Messages and queuing prompts for broadcast messaging',
  admin_text_cat_desc_admin_direct: '📝 Prompts for sending direct messages to specific users',
  admin_text_cat_desc_admin_other: '📝 Alerts and general administrative system copy',
  admin_text_cat_desc_menus: '📝 Button labels, headings, and main navigation menus',
  admin_text_cat_desc_user_purchase:
    '📝 Messages for purchasing, renewing, issuing, and subscription status',
  admin_text_cat_desc_user_wallet:
    '📝 Wallet messages, top-up instructions, and receipt submission',
  admin_text_cat_desc_user_general:
    '📝 Welcome messages, general user copy, support, and permissions',
  admin_broadcast_audience_prompt: '🎯 Choose the broadcast audience:',
  admin_broadcast_title: 'Broadcast Center',
  admin_broadcast_subtitle: 'Choose a customer segment and review exactly what will be queued.',
  admin_broadcast_audience_section: 'Audience',
  admin_broadcast_recipient_count_label: 'Recipients now',
  admin_broadcast_compose_title: 'Compose Broadcast',
  admin_broadcast_compose_subtitle:
    'The final message will be delivered exactly as entered after your confirmation.',
  admin_broadcast_preview_title: 'Review Broadcast',
  admin_broadcast_preview_subtitle:
    'Confirm the audience and message before it enters the durable delivery queue.',
  admin_broadcast_message_section: 'Message preview',
  admin_message_preview_truncated:
    'The preview is shortened here; the complete message will be sent exactly as entered.',
  admin_broadcast_queued_title: 'Broadcast Queued',
  admin_broadcast_queued_subtitle:
    'Delivery continues safely in the background. You can inspect progress or request cancellation below.',
  admin_broadcast_audience_active: '🟢 Active subscriptions',
  admin_broadcast_audience_none: '📭 No subscription',
  admin_broadcast_audience_30d: '🕐 No purchase in 30 days',
  admin_broadcast_audience_inactive: '🔴 No active subscription',
  admin_broadcast_audience_all: 'All non-banned users',
  admin_broadcast_audience_active_subscription: 'Users with an active subscription',
  admin_broadcast_audience_no_subscription: 'Users without any subscription',
  admin_broadcast_audience_no_purchase_30d: 'Users with no completed purchase in the last 30 days',
  admin_broadcast_audience_no_active_subscription: 'Users with subscriptions but none active',
  admin_broadcast_empty_audience: 'ℹ️ No users currently match this audience.',
  admin_broadcast_prompt_for_audience:
    '💬 Audience: {audience}\nRecipients now: {recipient_count}\n\nEnter the broadcast message:',
  admin_broadcast_too_long: '⚠️ Telegram broadcast text must be at most 4096 characters.',
  admin_broadcast_preview:
    '📣 Broadcast preview\n\nAudience: {audience}\nRecipients: {recipient_count}\n\n———\n{message}\n———\n\nQueue this broadcast?',
  admin_broadcast_confirm_button: '✅ Queue broadcast',
  admin_broadcast_queued:
    '📣 Broadcast was queued for {recipient_count} recipients. Delivery continues safely in the background.',
  admin_broadcast_status_button: '📊 Status',
  admin_broadcast_cancel_button: '⛔ Cancel',
  admin_broadcast_refresh_button: '🔄 Refresh',
  admin_broadcast_job_missing: '⚠️ Broadcast job not found.',
  admin_broadcast_cancel_requested: '⛔ Cancellation requested.',
  admin_broadcast_cancel_unavailable: 'ℹ️ This broadcast is already terminal or unavailable.',
  admin_broadcast_status_queued: 'Queued',
  admin_broadcast_status_running: 'Sending',
  admin_broadcast_status_cancel_requested: 'Cancel requested',
  admin_broadcast_status_cancelled: 'Cancelled',
  admin_broadcast_status_completed: 'Completed',
  admin_broadcast_status:
    '📣 *Broadcast* `{uuid}`\n\nAudience: {audience}\nStatus: *{status}*\nProcessed: {processed_count}/{recipient_count}\nSent: {sent_count} | Failed: {fail_count}\nCreated: {created_at}',
  admin_broadcast_status_title: 'Broadcast Status',
  admin_broadcast_status_subtitle: 'Live delivery counters for the queued broadcast.',
  admin_broadcast_id_label: 'Broadcast ID',
  admin_broadcast_audience_label: 'Audience',
  admin_broadcast_processed_label: 'Processed',
  admin_broadcast_sent_label: 'Delivered',
  admin_broadcast_failed_label: 'Failed',
  admin_broadcast_created_label: 'Created',
  admin_direct_telegram_id_prompt: '💬 Enter the recipient Telegram ID:',
  admin_direct_title: 'Direct Message',
  admin_direct_subtitle: 'Send one reviewed message to a specific customer.',
  admin_direct_recipient_label: 'Customer Telegram ID',
  admin_direct_compose_title: 'Compose Direct Message',
  admin_direct_preview_title: 'Review Direct Message',
  admin_direct_message_section: 'Message preview',
  admin_direct_sent_title: 'Direct Message Sent',
  admin_direct_invalid_telegram_id: '⚠️ The recipient Telegram ID is invalid.',
  admin_direct_user_not_found: '⚠️ User not found.',
  admin_direct_message_prompt: '💬 Enter the direct message:',
  admin_direct_preview: 'Message preview for user {telegram_id}:\n\n{message}',
  admin_direct_confirm_button: '✅ Send message',
  admin_direct_sent: '✅ Message sent successfully.',
  admin_direct_failed: '⚠️ Failed to send message.',
  admin_alert_title: '⚠️ System alert\n{message}',
  admin_panel_outage:
    '⚠️ Rebecca panel “{panel}” is unavailable.\nEndpoint: `{endpoint}`\nFailed attempts: {attempts}',
  admin_panel_outage_title: 'Rebecca Panel Alert',
  admin_panel_outage_subtitle: 'A Rebecca panel is unavailable and needs operator attention.',
  admin_panel_outage_attempts_label: 'Failed connection attempts',

  renewal_notification: '⚠️ Subscription {username} needs renewal.\n{reasons}',
  renewal_notification_title: 'Renewal Needed',
  renewal_notification_subtitle: 'This service needs attention to keep working.',
  renewal_service_label: 'Service',
  renewal_attention_section: 'What needs attention',
  renewal_reason_low_traffic: 'Remaining data: {remaining}',
  renewal_reason_near_expiry: 'Time remaining: {remaining} days',
  operation_failed: 'Operation failed',
  menu_my_configs: 'My Subscriptions',
  support_not_configured: 'Support contact has not been configured yet.',
  admin_setting_not_configured: 'Not configured',
  admin_setting_group_support: '🛟 Support',
  admin_setting_group_support_desc: 'Telegram account that responds to users',
  admin_setting_support_destination: 'Support Account',
  admin_setting_support_destination_prompt:
    'Enter Telegram username (with or without @) or numeric user ID for support.',
  admin_setting_support_current: 'Current account: `{current}`',
  admin_support_change: '✏️ Change Support',
  admin_support_remove: '🗑 Remove Support',
  referral_share_button: '📤 Share Referral Link',
  topup_change_receipt_button: '🔄 Change Receipt',
  traffic_unavailable: 'Unknown / Unavailable',
  admin_text_invalid_markdown: 'Telegram Markdown syntax is invalid',
  support_message: '💬 Use the button below to contact support directly.',
  support_title: 'Support',
  support_contact_button: '💬 Contact support',
  support_subtitle: 'Need help with a purchase, payment, or connection?',

  admin_setting_refund_window_hours: 'Unused-service refund window (hours; 0 = unlimited)',
  config_delete_refund_confirm_button: '💸 Delete & refund {amount}',
  config_delete_without_refund_button: '🗑 Delete without refund',
  config_delete_review_title: 'Delete Service',
  config_delete_refund_subtitle:
    'Review the wallet refund before permanently deleting this service.',
  config_delete_no_refund_subtitle: 'This service is not eligible for an automatic refund.',
  config_delete_service_label: 'Service',
  config_delete_original_charge_label: 'Original charge',
  config_delete_cashback_label: 'Cashback withheld',
  config_delete_refund_label: 'Refund to wallet',
  config_delete_eligibility_label: 'Refund status',
  config_delete_refund_consequence:
    'This permanently removes the remote service and returns the stated amount to your wallet.',
  config_delete_no_refund_consequence:
    'This permanently removes the remote service. It cannot be undone and no refund will be issued.',
  config_delete_success_title: 'Service Deleted',
  config_delete_success_refund_subtitle:
    'The service was removed and the refund was added to your wallet.',
  config_delete_success_no_refund_subtitle:
    'The service was permanently removed. No refund was issued.',
  config_delete_refund_warning:
    '⚠️ *Delete subscription* `{username}`\n\nPurchase charge: {gross_amount}\nCashback withheld: {cashback_withheld}\n💸 Refund to wallet: *{refund_amount}*\n\nRebecca reports zero current and lifetime usage. Delete & Refund removes the remote service and credits the amount above.',
  config_delete_no_refund_warning:
    '⚠️ *Delete subscription* `{username}`\n\nThis service is not eligible for an automatic refund:\n{reason}\n\nYou can still permanently delete the service without a refund.',
  refund_reason_config_not_found: 'The subscription does not belong to this account.',
  refund_reason_remote_unavailable: 'Rebecca cannot confirm the service state.',
  refund_reason_already_used: 'The subscription has already been used.',
  refund_reason_not_purchased_here: 'This subscription was not purchased through the bot.',
  refund_reason_ownership_mismatch:
    'Rebecca ownership marker does not match this purchase; manual review is required.',
  refund_reason_renewed_service:
    'The subscription has already been renewed and requires manual refund review.',
  refund_reason_refund_window_expired: 'The refund window for this purchase has expired.',
  refund_reason_referral_reward_attached:
    'This purchase generated a referral reward and requires manual admin review.',
  refund_reason_already_refunded: 'This purchase has already been refunded.',
  refund_reason_refund_in_progress: 'This refund is already pending reconciliation.',
  config_refund_not_eligible: '⚠️ Refund unavailable: {reason}',
  config_refunded_deleted:
    '✅ Subscription `{username}` was deleted and *{amount}* was returned to your wallet.',
  config_refund_pending:
    '⏳ Rebecca deletion outcome is uncertain. The refund intent is saved and will be reconciled without double payment.',
  config_refund_failed: '⚠️ Delete & Refund failed. Your wallet was not changed.',
  subscription_transfer_button: '🔁 Transfer service',
  transfer_config_missing:
    '⚠️ The selected subscription is no longer available. Start again from My Subscriptions.',
  transfer_target_prompt:
    '👤 Send the destination Telegram ID or @username. The recipient must have started the bot before.',
  transfer_title: 'Transfer Service',
  transfer_subtitle: 'Choose another existing bot user to receive this service.',
  transfer_target_hint: 'Send the recipient’s Telegram username or numeric ID.',
  transfer_review_title: 'Review Service Transfer',
  transfer_review_subtitle:
    'Confirm the recipient before the service is permanently moved to their account.',
  transfer_consequence: 'You will no longer be able to manage this service after the transfer.',
  transfer_success_title: 'Service Transferred',
  transfer_success_subtitle: 'The recipient can now manage this service from their account.',
  transfer_recipient_label: 'Recipient',
  transfer_recipient_id_label: 'Telegram ID',
  transfer_service_label: 'Transferred service',
  transfer_target_not_found: '⚠️ The destination user was not found in the bot.',
  transfer_target_same_user: '⚠️ This subscription already belongs to that user.',
  transfer_target_banned: '⚠️ A subscription cannot be transferred to a banned user.',
  transfer_confirm_button: '✅ Transfer service',
  transfer_confirm_prompt:
    '🔁 Transfer this subscription to:\n\nTelegram ID: `{telegram_id}`\nUsername: {username}\n\nAfter confirmation it will leave the current account.',
  transfer_success: '✅ Subscription `{username}` was transferred to user {telegram_id}.',
  transfer_recipient_notice:
    '🎁 Subscription `{username}` was transferred from user {telegram_id} to your account and is now available in My Subscriptions.',
  transfer_recipient_notice_title: 'A Service Was Added to Your Account',
  transfer_recipient_notice_subtitle: 'You can now manage this service from My Subscriptions.',
  transfer_sender_label: 'Transferred by',
  transfer_failed: '⚠️ Transfer failed. Ownership was not changed.',
  admin_menu_admins: '🛡️ Admins',
  admin_menu_orphans: '🧩 Orphan services',
  admin_admins_title: '🛡️ *Administrator Management*\n\n👥 Active administrators: {count}',
  admin_registry_title: 'Administrator Registry',
  admin_registry_subtitle: 'Manage trusted operators with access to the bot control surface.',
  admin_registry_count_label: 'Active administrators',
  admin_registry_section: 'Operator accounts',
  admin_registry_empty_title: 'No Administrator Records',
  admin_registry_empty_body: 'Add a trusted operator before relying on delegated administration.',
  admin_registry_id_label: 'Telegram ID',
  admin_remove_admin_title: 'Remove Administrator',
  admin_remove_admin_subtitle: 'Review the operator account before removing access.',
  admin_remove_admin_consequence:
    'This operator will immediately lose access to the admin controls. Their historical audit records stay intact.',
  admin_admin_removed_title: 'Administrator Removed',
  admin_admin_removed_subtitle: 'The operator no longer has access to the bot control surface.',
  admin_admin_you: 'you',
  admin_add_admin_button: '➕ Add admin',
  admin_remove_admin_button: '➖ Remove',
  admin_add_admin_prompt: 'Send the numeric Telegram ID of the new administrator:',
  admin_admin_added: '✅ User {telegram_id} is now an administrator.',
  admin_admin_already_exists: 'ℹ️ User {telegram_id} is already an administrator.',
  admin_remove_admin_confirm: 'Remove administrator access from {telegram_id}?',
  admin_admin_removed: '✅ Administrator {telegram_id} was removed.',
  admin_admin_not_found: '⚠️ Administrator {telegram_id} was not found.',
  admin_last_admin_cannot_remove:
    '⚠️ The last administrator cannot be removed. Add another admin first.',
  admin_orphan_none: '✅ No open Rebecca/database reconciliation issues were found.',
  admin_orphan_scan_button: '🔄 Scan now',
  admin_orphan_scanning: '🔄 Rebecca reconciliation scan started…',
  admin_orphan_scan_done:
    '✅ Scan complete. Local→Missing: {local} | Remote→Unbound: {remote} | Baseline/ignored: {ignored}',
  admin_orphan_scan_partial:
    '⚠️ Healthy panels were scanned; {failed} panel(s) were unavailable. Local missing: {local} | Remote unbound: {remote} | Ignored: {ignored}',
  admin_orphan_scan_failed: '⚠️ Scan did not complete. Check Rebecca and database connectivity.',
  admin_orphan_baseline_button: '🧱 Establish current Rebecca baseline',
  admin_orphan_baseline_confirm:
    '🧱 *Establish current Rebecca baseline?*\n\nEvery Rebecca service that exists *right now* but is not linked to the bot will be marked as pre-existing/manual and ignored by future orphan scans.\n\nNothing is deleted, refunded, assigned, or changed in Rebecca. New services created later will still be detected. Use this after installing the reconciliation feature on a panel that already contains manual services.',
  admin_orphan_baseline_confirm_button: '✅ Establish baseline',
  admin_orphan_baseline_running: '🧱 Reading the current Rebecca service list…',
  admin_orphan_baseline_done:
    '✅ Baseline established. Rebecca services: {remote} | Already linked: {bound} | Marked pre-existing/manual: {ignored}',
  admin_orphan_baseline_failed:
    '⚠️ Baseline was not established. No services were changed; check Rebecca/database connectivity and try again.',
  admin_orphan_summary: '🧩 Open issues: {total}',
  admin_orphan_card:
    '🧩 *Subscription drift*\nType: {kind}\nService: `{username}`\nLocal owner: `{owner}`\nFirst seen: {first_seen}',
  admin_orphan_kind_local: 'Local → Rebecca Missing',
  admin_orphan_kind_remote: 'Rebecca → Local Unbound',
  admin_orphan_remove_local_button: '🗑 Remove local record',
  admin_orphan_assign_button: '👤 Assign to user',
  admin_orphan_ignore_button: '🙈 Ignore',
  admin_orphan_assign_prompt:
    'Send the destination Telegram ID, @username, or one of that user’s service usernames:',
  admin_orphan_assigned: '✅ Subscription `{username}` was assigned to user {telegram_id}.',
  admin_orphan_assign_failed: '⚠️ Failed to assign the orphan subscription.',
  admin_orphan_issue_missing: '⚠️ This issue is no longer open or available.',
  admin_orphan_remove_confirm:
    'Remove local record `{username}`? This does not delete any remote service and does not issue a refund.',
  admin_orphan_removed: '✅ The local orphan record was removed.',
  admin_orphan_remote_reappeared:
    'ℹ️ The service exists in Rebecca again. The stale issue was closed and nothing was deleted.',
  admin_orphan_remove_failed: '⚠️ Failed to remove the local orphan record. Try scanning again.',
  admin_orphan_ignored: '✅ Issue ignored. Future scans will not reopen it automatically.',
  admin_orphan_queue_title: 'Subscription Reconciliation',
  admin_orphan_queue_subtitle:
    'Resolve differences between Rebecca panels and the bot records one service at a time.',
  admin_orphan_queue_pending_label: 'Open reconciliation issues',
  admin_orphan_queue_section: 'Issues on this page',
  admin_orphan_queue_empty_title: 'No Reconciliation Issues',
  admin_orphan_queue_empty_body:
    'Bot records and reachable Rebecca panels are currently in sync. You can run a fresh scan when needed.',
  admin_orphan_detail_title: 'Subscription Drift Detail',
  admin_orphan_kind_label: 'Drift type',
  admin_orphan_service_label: 'Service username',
  admin_orphan_owner_label: 'Local owner',
  admin_orphan_first_seen_label: 'First detected',
  admin_orphan_scan_result_title: 'Reconciliation Scan Complete',
  admin_orphan_local_missing_label: 'Local records missing in Rebecca',
  admin_orphan_remote_unbound_label: 'Rebecca services without a local owner',
  admin_orphan_ignored_label: 'Ignored manual/baseline services',
  admin_orphan_failed_panels_label: 'Panels unavailable during scan',
  admin_orphan_baseline_title: 'Establish Rebecca Baseline',
  admin_orphan_baseline_subtitle:
    'Mark already-existing unbound services as manual so future scans focus on new drift.',
  admin_orphan_baseline_consequence:
    'No remote service, customer balance, or local subscription is deleted or reassigned by this action.',
  admin_orphan_baseline_result_title: 'Rebecca Baseline Recorded',
  admin_orphan_remote_total_label: 'Rebecca services found',
  admin_orphan_already_bound_label: 'Already linked to the bot',
  admin_orphan_manual_baseline_label: 'Marked as manual/pre-existing',
  admin_orphan_remove_title: 'Remove Stale Local Record',
  admin_orphan_remove_subtitle:
    'Review the affected subscription before removing only its local record.',
  admin_orphan_remove_consequence:
    'This does not delete the remote Rebecca service and does not issue a refund.',
  purchase_confirmation_expired:
    '⌛️ This confirmation expired or was already used. Start again from the shop.',
  admin_menu_panels: '🖥 Rebecca panels',
  admin_panels_title: '🖥 Rebecca panel management\n\nPanels: {count}',
  admin_panel_registry_title: 'Rebecca Panels',
  admin_panel_registry_subtitle:
    'Select a panel to inspect its connection, services, and operational settings.',
  admin_panel_registry_total_label: 'Configured panels',
  admin_panel_registry_empty_title: 'No Rebecca Panels Yet',
  admin_panel_registry_empty_subtitle: 'Add the first panel before offering subscriptions.',
  admin_panel_detail_title: 'Panel Details',
  admin_panel_test_title: 'Panel Connection Check',
  admin_panel_test_subtitle:
    'A live health check was completed against the configured Rebecca endpoint.',
  admin_panel_api_key_title: 'Secure API Key Entry',
  admin_panel_api_key_subtitle:
    'Enter the key in your next message. It is encrypted immediately and never shown again.',
  admin_panel_saved_title: 'Panel Settings Saved',
  admin_panel_saved_subtitle: 'The operational setting was updated successfully.',
  admin_panel_save_failed_title: 'Panel Update Failed',
  admin_panel_save_failed_subtitle:
    'No setting was changed. Check the supplied value and try again.',
  admin_panel_connection_section: 'Connection & Role',
  admin_panel_services_section: 'Services',
  admin_panel_name_label: 'Panel name',
  admin_panel_status_label: 'Status',
  admin_panel_default_label: 'Default panel',
  admin_panel_endpoint_label: 'Endpoint',
  admin_panel_credential_label: 'Credential mode',
  admin_panel_default_service_label: 'Default service',
  admin_panel_service_id_label: 'Service ID',
  admin_panel_no_services_label: 'No service targets configured',
  admin_panel_delete_title: 'Delete Panel',
  admin_panel_delete_subtitle:
    'Panel deletion is blocked when operational records still depend on it.',
  admin_panel_delete_consequence:
    'If deletion is allowed, this panel and its service targets will be removed permanently.',
  admin_panel_service_delete_title: 'Delete Panel Service',
  admin_panel_service_delete_subtitle:
    'This service target will no longer be selectable for new sales.',
  admin_panel_service_delete_consequence:
    'Deleting the target does not delete customers’ remote services.',
  admin_panels_empty: '🖥 No panel is configured yet. Add your first panel.',
  admin_panel_add_button: '➕ Add panel',
  admin_panel_name_prompt: '🏷 Send a short, recognizable panel name:',
  admin_panel_url_prompt: '🌐 Send the HTTPS panel origin without an extra path:',
  admin_panel_api_key_prompt:
    '🔐 Send the API key. It will be encrypted and never displayed again.',
  admin_panel_service_id_prompt: '🔢 Send the numeric service ID:',
  admin_panel_service_name_prompt: '🏷 Send a short service name:',
  admin_panel_saved: '✅ Panel settings saved.',
  admin_panel_save_failed: '⚠️ Save failed. Check the URL, credential, and values.',
  admin_panel_required_first: '⚠️ Add an enabled panel and at least one service first.',
  admin_pkg_target_prompt: '🎯 Select the panel and service for this package:',
  admin_panel_not_found: '⚠️ Panel not found.',
  admin_panel_detail:
    '🖥 {name}\n\nStatus: {status}\nDefault panel: {default_status}\nURL: {url}\nCredential: {credential}\nDefault service: {service}',
  admin_panel_status_enabled: '🟢 Enabled',
  admin_panel_status_disabled: '⚪️ Disabled',
  admin_panel_credential_api_key: '🔐 API key',
  admin_panel_credential_password: '🔐 Username/password',
  admin_panel_credential_none: '⚠️ Not configured',
  admin_panel_test_button: '🩺 Test connection',
  admin_panel_test_ok: '✅ Panel connection is healthy.',
  admin_panel_test_failed: '⚠️ Connection failed. Check the URL and access.',
  admin_panel_enable_button: '▶️ Enable',
  admin_panel_disable_button: '⏸ Disable',
  admin_panel_make_default_button: '⭐ Make default',
  admin_panel_edit_name_button: '✏️ Name',
  admin_panel_edit_url_button: '🌐 URL',
  admin_panel_edit_key_button: '🔑 API key',
  admin_panel_add_service_button: '➕ Service',
  admin_panel_service_default_button: '⭐ Default',
  admin_panel_custom_target_button: '🎯 Custom volume',
  admin_panel_custom_target_selected_button: '✅ Custom-volume target',
  admin_panel_custom_target_saved: '✅ Custom-volume target saved.',
  admin_panel_service_delete_button: '🗑 Delete',
  admin_panel_service_delete_confirm:
    'Delete the “{service}” service from this panel? This cannot be undone.',
  admin_panel_delete_button: '🗑 Delete panel',
  admin_panel_delete_confirm:
    '⚠️ Delete this panel? Deletion is blocked when purchases, subscriptions, or history reference it.',
  admin_panel_deleted: '✅ Panel deleted.',
  admin_panel_in_use: '⚠️ This panel or service is referenced by bot data and cannot be deleted.',
  admin_panel_last_delete: '⚠️ The last panel cannot be deleted.',
  private_chat_only: '🔒 For privacy, use this bot only in a private chat.',
};
